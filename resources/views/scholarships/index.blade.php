...
<div class="container mx-auto px-8 mt-12 max-w-[1280px]" x-data="{ showFilters: false }" x-init="showFilters = (sessionStorage.getItem('showFilters') === 'true')">
    <button @click="showFilters = !showFilters; sessionStorage.setItem('showFilters', showFilters)" class="bg-Second text-white px-4 py-4 rounded mb-4 flex items-center">
    ...
    <div x-show="showFilters" class="mb-4 flex flex-col space-y-4">
        <form action="{{ route('scholarships.index') }}" method="GET" class="space-y-4">
            @csrf
            <div class="flex flex-col md:flex-row items-center justify-between">
                <!-- Filter by Provider -->
                <div class="flex items-center mb-2 md:mb-0">
                    <label for="provider" class="font-Fredoka mb-2 mr-2 text-Second">Filter by Provider:</label>
                    <select id="provider" name="provider" class="block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:border-blue-500">
                ...
                <div class="flex items-center mb-2 md:mb-0">
                    ...
                    <select id="international" name="international" class="block py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:border-blue-500">
                ...
                <div class="flex items-center mb-2 md:mb-0">
                    ...
                    <select id="gender" name="gender" class="block py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:border-blue-500">
                        <option value="">All</option>
                ...
                <div class="flex items-center">
                    <a href="{{ route('scholarships.index', ['sort' => 'provider'] + request()->except('sort')) }}" class="text-Second hover:text-blue-700 ml-0 md:ml-4">
...
<div id="scholarshipsList" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 max-w-[1280px] mx-auto px-4">
    @if($scholarships->isEmpty())
        <div class="col-span-full text-center text-gray-500">
            <p>No scholarships match your current filters. Please try adjusting your search criteria.</p>
        </div>
    @else
            ...
            <div class="bg-white shadow-md rounded-lg overflow-hidden flex flex-col" data-provider="{{ strtolower($scholarship->provider) }}">
                <div class="p-4 flex justify-center items-center bg-gray-100 h-60 w-60 mx-auto">
                    <div class="h-full w-full flex items-center justify-center">
                        @include('components.scholarship-logo', ['provider' => strtolower($scholarship->provider)])
                    </div>
                </div>
                <div class="flex-1 p-4 flex flex-col justify-between">
                    <div>
                        <p class="text-sm font-medium text-Second">{{ $scholarship->provider }}</p>
