...
<header class="">
    ...
    <div
        class="md:hidden flex items-center justify-between fixed top-0 w-full h-16 px-4 bg-opacity-50 backdrop-blur-sm z-50 bg-Button/50">
    ...
    {{-- <div class="hidden md:flex justify-center items-center static h-fit">
        <div class="z-50 px-4 flex fixed top-8 self-center backdrop-blur-sm bg-Button/50 rounded-full">
            <nav class="flex self-center space-x-4 text-white font-Fredoka py-4 text-center">
                <a href="{{ route('home') }}"
                ...
                <a href="{{ route('scholarships.index') }}"
                    class="{{ request()->is('scholarships*') ? 'bg-Button' : '' }} hover:backdrop-blur-none hover:bg-Button rounded-3xl px-4 py-1 flex flex-col justify-center">
                    <div>Find </div>
